lines =[]
valid = 0
with open("/Users/pmaharjan/Desktop/advent of code/day2/input.txt") as file:
	for line in file:
		line = line.strip()
		lines.append(line)
...
for l in lines: # 15-16 f: ffffffffffffffhf
        ...
        while(i < len(list)):
                ...
                pattern = list[2]
                #print("pattern: {0}".format(pattern))
                if((pattern[int(positionone)-1] == letter and pattern[int(positiontwo)-1] != letter) or (pattern[int(positionone)-1] != letter and pattern[int(positiontwo)-1] == letter)):
			part2 += 1
